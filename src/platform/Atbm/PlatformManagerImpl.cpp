...
extern "C" {
extern int hal_random_get(void);
}

static int app_entropy_source(void * data, unsigned char * output, size_t len, size_t * olen)
{
    ChipLogError(DeviceLayer, "%s:%d", __func__, __LINE__);

    unsigned int *p = (unsigned int *)output;
    size_t i = 0;

    *olen = len;
    len /= 8;
    if(((unsigned int)p)%sizeof(unsigned int))
    {
        p = p + (sizeof(unsigned int) - ((unsigned int)p)%sizeof(unsigned int));
        len = len - ((unsigned int)p)%sizeof(unsigned int);
    }
...
CHIP_ERROR PlatformManagerImpl::_InitChipStack(void)
{
    ...
    ChipLogError(DeviceLayer, "_InitChipStack %s:%d", __func__, __LINE__);
    CHIP_ERROR err;
